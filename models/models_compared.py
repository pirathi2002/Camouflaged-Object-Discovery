...
from data.utils import convert_mapping_to_flow, convert_flow_to_mapping
...
class GLU_Net:
    def __init__(self, model_type='DPED_CityScape_ADE', path_pre_trained_models='pre_trained_models/',
        ...
        try:
            net.load_state_dict(torch.load(checkpoint_fname, map_location=device))
        except:
    ...
    def estimate_flow(self, source_img, target_img, device, mode='channel_first'):
        if self.apply_flipping_condition:
            ...
            if self.net.target_image_is_flipped:
                flipped_mapping = convert_flow_to_mapping(flow_original_reso, output_channel_first=True).permute(0, 2, 3, 1).cpu().detach().numpy()
